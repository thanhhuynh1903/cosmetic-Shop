...
export default function Header() {
  ...
  const mobileNavItemStyle = ({ isActive }) =>
    isActive
      ? "block py-3 border-b border-gray-200 text-center font-medium"
      : "block py-3 border-b border-gray-200 text-center"

  return (
    <div className={`color-header w-full sticky top-0 z-50 ${scrolled ? "shadow-md" : ""}`}>
      <div className="w-full md:w-[88%] h-full mx-auto px-4 md:px-0">
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white text-gray-800 py-2 px-4 shadow-lg rounded-b-lg absolute left-0 right-0 z-50 transition-all duration-300 ease-in-out">
            <NavItem className={mobileNavItemStyle} to={routes.home} onClick={() => setIsMenuOpen(false)}>
              Home
            </NavItem>
            <NavItem className={mobileNavItemStyle} to={routes.products} onClick={() => setIsMenuOpen(false)}>
              Products
            </NavItem>
            <NavItem className={mobileNavItemStyle} to={routes.about} onClick={() => setIsMenuOpen(false)}>
              About
            </NavItem>
            <NavItem className={mobileNavItemStyle} to={routes.blog} onClick={() => setIsMenuOpen(false)}>
              Blog
            </NavItem>
            <NavItem className={mobileNavItemStyle} to={routes.content} onClick={() => setIsMenuOpen(false)}>
              Content
            </NavItem>
          </div>
        )}
